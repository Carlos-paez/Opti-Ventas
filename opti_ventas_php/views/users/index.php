    ...
    <form method="GET" class="flex flex-col sm:flex-row gap-3 mb-6">
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="<?= e($filters['search']) ?>" placeholder="Buscar por nombre o email..."
                   class="w-full pl-10 pr-4 py-2 border-gray-300 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500">
        </div>
        <select name="role" class="border-gray-300 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500">
            <option value="">Todos los roles</option>
            <option value="admin" <?= $filters['role'] === 'admin' ? 'selected' : '' ?>>Admin</option>
            <option value="seller" <?= $filters['role'] === 'seller' ? 'selected' : '' ?>>Vendedor</option>
        </select>
        <button type="submit" class="inline-flex items-center justify-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 transition">
            Filtrar
        </button>
    </form>
                    ...
                    <?php foreach ($users as $user): ?>
                        <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <?php else: ?>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Vendedor</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <?php if ((int) $user['id'] !== Auth::id()): ?>
                                        <form method="POST" action="<?= url('/users/' . (int) $user['id'] . '/toggle-active') ?>" class="inline">
                                            <?= csrf_field() ?>
                                            <button type="submit"
                                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg border transition <?= $user['is_active'] ? 'text-red-700 bg-red-50 border-red-200 hover:bg-red-100' : 'text-green-700 bg-green-50 border-green-200 hover:bg-green-100' ?>">
                                                <?php if ($user['is_active']): ?>
                                                    <svg class="w-3.5 h-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
