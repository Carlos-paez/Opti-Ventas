...
<div class="flex items-center gap-3">
    ...
    <h2 class="text-xl font-bold text-gray-900 dark:text-white">Editar Categoría</h2>
</div>
<?php View::endSection(); ?>

<div class="max-w-2xl">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
        <form method="POST" action="<?= url('/categories/' . (int) $category['id']) ?>">
            <?= csrf_field() ?>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="<?= e(old('name', $category['name'])) ?>" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              placeholder="Descripción opcional"><?= e(old('description', $category['description'])) ?></textarea>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" id="color" name="color" value="<?= e(old('color', $category['color'])) ?>"
                               class="w-10 h-10 rounded-lg border border-gray-300 dark:border-gray-600 cursor-pointer">
                        <span id="color-value" class="text-sm text-gray-500 dark:text-gray-400 font-mono"><?= e(old('color', $category['color'])) ?></span>
                    </div>
